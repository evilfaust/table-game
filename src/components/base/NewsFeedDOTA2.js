...
const NewsFeedDOTA2 = () => {
  ...
  const fetchNews = async () => {
    try {
      ...
      const response = await fetch(`${proxyUrl}?groupId=224716938&count=50`);
  ...
  if (error) {
    return <Alert message="Ошибка" description={error} type="error" style={{ textAlign: 'center', marginTop: '20px' }} />;
  }

  return (
    <Layout style={{ background: 'transparent' }}>
      <Layout.Content style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', width: '95%' }}>
        <h2 style={{ textAlign: "center" }}>Новости DOTA 2</h2>
