...
const TournamentBracket = ({ tournamentIdentifier }) => {
  const [tournamentData, setTournamentData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  console.log('TournamentBracket получил tournamentIdentifier:', tournamentIdentifier);
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        ...
        // Используем абсолютный URL для запроса к прокси-серверу
        const url = `https://emcotech.site:5551/fetch-tournament?tournamentIdentifier=${tournamentIdentifier}`;
        console.log('Запрос турнира по URL:', url);

        const response = await fetch(url);
        const text = await response.text();
        console.log('Полученный ответ (как текст):', text);

        // Пробуем распарсить JSON, если ответ не валиден — выведем ошибку
        try {
    ...
    if (tournamentIdentifier) {
      ...
      setError('tournamentIdentifier не передан в компонент');
      setLoading(false);
    }
  }, [tournamentIdentifier]);

  if (loading) return <div>Загрузка данных турнира...</div>;
  if (error) return <div>Ошибка: {error}</div>;

  const tournament = tournamentData?.data?.tournament;
  if (!tournament) {
    return <div>Нет данных по турниру</div>;
  }
      ...
      {tournament.matches && tournament.matches.length > 0 ? (
        <ul>
          {tournament.matches.map(match => (
            <li key={match.id}>
              {match.player1_name} vs {match.player2_name} – {match.score || 'не сыгран'}
            </li>
          ))}
        </ul>
      ) : (
        <p>Нет данных по матчам.</p>
      )}
    </div>
  );
};
